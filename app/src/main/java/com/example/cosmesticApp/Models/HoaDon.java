package com.example.cosmesticApp.Models;

import android.util.Log;

import androidx.annotation.NonNull;

import com.example.cosmesticApp.my_interface.IHoaDon;
...
public class HoaDon implements Serializable {
    ...
    public void HandleReadData(String iduser, int positionStatus) {
        ...
        if(positionStatus==0 && iduser == null) {
            db.collection("HoaDon").orderBy("ngaydat", Query.Direction.DESCENDING)
                    .get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                @Override
                public void onSuccess(@NonNull QuerySnapshot queryDocumentSnapshots) {
                    if (queryDocumentSnapshots.size() > 0) {
